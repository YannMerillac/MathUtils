...
c                                                 
c============================================================================= 
      subroutine setulb(n, m, x, l, u, nbd, f, g, factr, pgtol, fdelta,
...
c
c-jlm-jn
     +                 wa(2*m*n + 5*n + 11*m*m + 8*m), dsave(29), fdelta
c     ************
c
c     Subroutine setulb
c
...
c-jlm-jn 
     ...
     +          lwy,lsy,lss,lwt,lwn,lsnd
      ...
      call mainlb(n,m,x,l,u,nbd,f,g,factr,pgtol,fdelta,
...
c======================= The end of setulb =============================
      ...
      subroutine mainlb(n, m, x, l, u, nbd, f, g, factr, pgtol,fdelta,
     +                  ws, wy,sy, ss, wt, wn, snd, z, r, d, t, xp, wa, 
     ...
     +                 iwhere(n), indx2(n), isave(23)
      double precision f, factr, pgtol,fdelta,
...
c     Generate the search direction d:=z-x.
 ...
 666  continue
     ...
     +            dtd,xstep,stpmx,fdelta,iter,ifun,iback,nfgv,info,
     +            task,boxed,cnstnd,csave,isave(22),dsave(17),iprint)
...
c                                terminate the algorithm.
      ...
      if (abs(fold - f) .le. tol*ddum) then
c                                        terminate the algorithm.
...
c====================== The end of hpsolb ==============================
     ...
     +                  z, stp, dnorm, dtd, xstep, stpmx, fdelta, iter,
     +                  ifun,iback, nfgv, info, task, boxed, cnstnd,
     +                  csave, isave, dsave, iprint)
...
c     Determine the maximum step length.
  ...
  43        continue
      ...
      if (iter .eq. 0) then
c         print*,'L-BFGS-B modified first step length according to expected
c     + function reduction :'
c        stp = min(one/dnorm, stpmx)	  initial code 
         if (boxed) then
             stp = fdelta
         else
             stp = min(fdelta/dnorm, stpmx)
         endif
c	     print*,stp
      else
        stp = one
